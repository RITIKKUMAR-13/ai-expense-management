...
export default function ExpenseWorkspace() {
  ...
  const renderBudgets = () => <div className="budget-stack">{budgets.length === 0 ? <div className="expense-empty"><Target size={25} /><p>No budgets set for this month.</p><small>Create a category budget to monitor spending pace.</small></div> : budgets.map((budget) => { const meta = categoryMeta[budget.category]; const percent = Math.min(100, Math.round((budget.spentPaise / budget.limitPaise) * 100)); return <div className="budget-item" key={budget.id}><div className="budget-item-head"><span><i style={{ background: meta?.color }} />{meta?.label || budget.category}</span><strong>{asCurrency(budget.spentPaise)} <small>/ {asCurrency(budget.limitPaise)}</small></strong></div><div className="budget-bar"><i style={{ width: `${percent}%`, background: meta?.color }} /></div><p>{percent}% used · {asCurrency(budget.limitPaise - budget.spentPaise)} remaining</p></div>; })}</div>;

  if (dashboard.isLoading) return <div className="expense-loading"><div className="expense-loading-mark"><WalletCards size={21} /></div><p>Spendwise AI</p><strong>Assembling your private<br />financial signal desk.</strong><span>Loading your authorized expense records…</span><i /></div>;
  ...
  return <DashboardLayout><div className="expense-workspace">
    ...
    {page === "/budgets" && <section className="expense-panel ledger-panel"><div className="expense-panel-head"><div><p className="expense-eyebrow">MONTHLY THRESHOLDS</p><h2>Budget studio</h2></div><BudgetDialog periodKey={periodKey} onSaved={refresh} /></div><div className="budget-main">{renderBudgets()}</div></section>}
